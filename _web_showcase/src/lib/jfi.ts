...
const BIN_PATH = path.join(PROJECT_ROOT, 'bin', 'jfi');

export interface JfiOptions {
  upcomingDays?: number;
  limit?: number;
  template?: string;
}
...
export function buildBinary() {
  console.log('[JFI Wrapper] Building binary...');
  spawnSync('make', ['build'], { cwd: PROJECT_ROOT });
}

export function runJfi(input: string | string[], options: JfiOptions = {}) {
...
function handleResult(result: any) {
  if (result.status !== 0) {
    const err = result.stderr?.toString() || result.error?.message || 'Binary execution failed';
    console.error(`[JFI Wrapper] Error: ${err}`);
    throw new Error(err);
  }
  ...
  try {
    ...
    console.error(`[JFI Wrapper] Parse Error. Output: ${output}`);
